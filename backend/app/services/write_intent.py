...
Company", "set price to 100 where category is Fruit". Reuses the same
column-resolution and filter grammar as nlq.py (read-only queries), so a
"where ..." clause parses identically on both sides.

Deliberately narrow: it only matches phrasings that name the target column
and the new value explicitly. A vaguer instruction like "change the Afresh
company name to XYZ" (value before column, filter implied by an example
value rather than a named column) doesn't match any pattern here and is
expected to fall through to the LLM classifier (ai_query.classify) instead —
...
def parse_write_intent(text: str, columns: list[str]) -> WriteIntent | None:
    ...
    if m:
        ...
        return WriteIntent(column=column, filters=filters, new_value=_parse_value(new_phrase))
